...
class TransactionTextAdapter(private val context: Context, private val list: List<Transactions>, private val layoutId: Int) : ListAdapter<Transactions, TransactionTextAdapter.TransactionTextViewHolder>(DiffCallback()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TransactionTextViewHolder {
        val textView = LayoutInflater.from(parent.context).inflate(layoutId, parent, false) as TextView
    ...
    override fun onBindViewHolder(holder: TransactionTextViewHolder, position: Int) {
        ...
        if(layoutId == R.layout.list_item_notification)
